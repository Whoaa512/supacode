...
ghostty_source_dir="${ghostty_build_root}/source"
...
ghostty_patch_path="${srcroot}/patches/ghostty-xcode-26.4.patch"
ghostty_legacy_prefix_path="${ghostty_source_dir}/zig-out"
...
print_fingerprint() {
  (
    cd "${ghostty_dir}"
    {
      ...
      shasum -a 256 "${ghostty_patch_path}" | awk '{print $1}'
...
rsync -a --delete \
  ...
  "${ghostty_dir}/" \
  "${ghostty_source_dir}/"
patch -s -d "${ghostty_source_dir}" -p1 < "${ghostty_patch_path}"
rm -rf "${ghostty_legacy_prefix_path}"
mkdir -p "${ghostty_legacy_prefix_path}"
...
"${zig_env[@]}" mise exec -- zig build -Doptimize=ReleaseFast -Demit-xcframework=true -Dxcframework-target=native -Demit-macos-app=false -Dsentry=false --prefix "${ghostty_build_root}" --cache-dir "${ghostty_local_cache_dir}" --global-cache-dir "${ghostty_global_cache_dir}"
rsync -a --delete "${ghostty_source_dir}/macos/GhosttyKit.xcframework/" "${xcframework_path}/"
prepare_xcframework
