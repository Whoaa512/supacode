...
struct CommandPaletteFeatureTests {
  ...
  @Test func commandPaletteItems_includesCollapseToggleWhenWorktreeSelected() {
    let rootPath = "/tmp/repo-collapse"
    let worktree = makeWorktree(id: "\(rootPath)/wt-1", name: "wt-1", repoRoot: rootPath)
    let repository = makeRepository(rootPath: rootPath, name: "MyRepo", worktrees: [worktree])
    var state = RepositoriesFeature.State(repositories: [repository])
    state.selection = .worktree(worktree.id)

    let items = CommandPaletteFeature.commandPaletteItems(from: state)

    let collapseItem = items.first {
      if case .toggleCollapseRepository = $0.kind { return true }
      return false
    }

    #expect(collapseItem != nil)
    #expect(collapseItem?.title == "Collapse Section: MyRepo")
  }

  @Test func commandPaletteItems_omitsCollapseToggleWithoutSelectedWorktree() {
    let rootPath = "/tmp/repo-no-select"
    let worktree = makeWorktree(id: "\(rootPath)/wt-1", name: "wt-1", repoRoot: rootPath)
    let repository = makeRepository(rootPath: rootPath, name: "MyRepo", worktrees: [worktree])
